...
plan.add((_this) => {
  _this.flyTimer = setInterval(() => fly.call(_this), 20); // 执行水平方向的动画
}).add((_this) => {
  _this.dropTimer = setInterval(() => drop.call(_this), 20); // 执行垂直方向的动画
});
...
function dragEnd(e) {
    ...
    // 设置垂直方向的下落初速度
    this.dropSpeed = 0;
    // 这里的this就是我们拖拽的元素对象,通过fire方法可以
    plan.fire(this); // 这里就是发布事件,因为这里已经到达了我们订阅的拖拽结束的时刻;
}
